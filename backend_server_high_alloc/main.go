...
import (
	...
	"github.com/sinta-p/tracker-app/backend_server_high_alloc/DatabaseManager"
	pb "github.com/sinta-p/tracker-app/backend_server_high_alloc/grpc"
...
// function to create heap alloc with object larger than 65535
...
type server struct {
	pb.UnimplementedTickerManager3Server
}
...
func main() {
	...
	tracer.Start(
		tracer.WithEnv("dev"),
		tracer.WithService("ticker-manager3"),
	...
	// set up profiler
	err := profiler.Start(
		profiler.WithService("ticker-manager3"),
	...
	si := grpctrace.StreamServerInterceptor(grpctrace.WithServiceName("ticker-manager3"))
	ui := grpctrace.UnaryServerInterceptor(grpctrace.WithServiceName("ticker-manager3"))

	// Create a listener for the server
	lis, err := net.Listen("tcp", fmt.Sprintf("backend-high-alloc:%d", *port))
	if err != nil {
		log.Fatalf("failed to listen: %v", err)
	}
	s := grpc.NewServer(grpc.StreamInterceptor(si), grpc.UnaryInterceptor(ui))
	pb.RegisterTickerManager3Server(s, &server{})
